	...
	public static void initConfig(String[] args) throws IOException {
		...
		if (args.length == 2 && args[0].equals("-c")) {
			...
			BufferedReader in = new BufferedReader(
					new InputStreamReader(ServerBootstrap.class.getClassLoader().getResourceAsStream("server.json")));
		...
		for (int i = 0; i < users.size(); i++) {
			...
			String basicAuth = Base64.getEncoder()
					.encodeToString((user.getString("UserName") + ":" + user.getString("Password")).getBytes());
